...
import "./interfaces/IHBTConfig.sol";
...
contract Pool is Ownable, ReentrancyGuard {
    ...
    // 0x0000...000 HBTCoin, other: ERC20
    address public fundToken;
    uint256 public fundTarget;
    uint256 public fundRaised;

    //TODO: Information on team allocation should be determined at this point
    ...
    // vesting info - always divided by 10 
    ...
    //TODO: setting up pool factory - Definition should be completed
    constructor(
    ...
    function getFeeInfo() private view returns (address, uint256) {
        IHBTConfig config = IHBTConfig(factory.config());
        //  Fees percentage is set to 5%
        // Fees recipient is set to multi-sig wallet - on receive transfer to appropriate
    ...
    // Let users withdraw xHBT after ending pool
    function withdrawFundedHBT(address payable to) external payable onlyOwner {
        require(block.timestamp > endTime, "Pool has not yet ended");
    ...
    function buyWithHBT() public payable checkBeforeBuy(msg.sender, msg.value) {
        require(fundToken == address(0), "It's not Kuc-buy pool!");
