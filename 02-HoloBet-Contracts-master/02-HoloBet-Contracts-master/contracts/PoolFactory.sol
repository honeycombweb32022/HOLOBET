...
contract PoolFactory is Ownable, IPoolFactory {
    ...
    constructor(IHBTConfig _config) {
        require(address(_config) != address(0), "Invalid config address");

        config = address(_config);
    }

    function setConfigAddress(IHBTConfig _config) external onlyOwner {
        require(address(_config) != address(0), "Invalid config address");
        config = address(_config);
    }
     ...
     * @param fundToken          Address of fundToken: 0x000.000: HBT, Other KRC20
     */
    function createPool(
        uint256 saleTarget,
        address fundToken,
